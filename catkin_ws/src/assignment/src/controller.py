...
from lib.sensors import PositionTracker, LaserScanner, Camera, DepthCamera, navigator
...
class TurtlebotController:
    ...
    def decide_behaviour(self):
        ...
        if self.navigator_switch and (self.nav_num == -1 or self.nav_num != self.nav_pre_num):
            self.state = 'navigating'  # Avoiding obstacles
            if self.state != self.state_previous:
                rospy.loginfo("have finished identying a target! forwarding next place...")
    ...
    def enact_behaviour(self):
        if self.state != self.state_previous:
            ...
            elif self.navigator_switch and self.state_previous == 'navigating':
                print("Cancelled navigating")
        ...
        if self.state == 'avoiding':
            ...
            if beacon_type != 0:  # If we reached the beacon
                ...
                #refresh navigator parameter
                if self.nav_num < 5:
                    self.navigator_switch = True
                if self.navigator_switch:
                    self.nav_pre_num = self.nav_num
                    self.nav_num += 1
                    if self.nav_num > 4:
                        self.navigator_switch = False
        elif self.navigator_switch and self.state == 'navigating':
            if self.nav_num == -1:
                self.nav_num += 1
            self.nav.navigate_to_point(self.nav_num)
            self.navigator_switch = False
